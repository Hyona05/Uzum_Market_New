    ...
    <div class="table-responsive">
        <table class="table table-striped">
            ...
            <tr id="seller-<%= id %>">
                ...
                <td>
                    <a href="/app/admin/showsellerprofile?id=<%= id %>" class="btn btn-primary" style="background-color: limegreen">Profile</a>
                </td>
                <td>
                    <button data-id="<%= id %>" data-status="<%= status %>"
                            class="btn <%= status.equals(Status.ACTIVE.name()) ? "btn-danger" : "btn-success" %> status-btn"
                            style="background-color: <%= status.equals(Status.ACTIVE.name()) ? "red" : "limegreen" %>">
                        <%= status.equals(Status.ACTIVE.name()) ? "Block" : "Unblock" %>
                    </button>
                </td>
                <td>
                    <button data-id="<%= id %>" class="btn btn-danger delete-btn" style="background-color: red">Delete</button>
                </td>
            </tr>
            <%
                    }
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.querySelectorAll('.status-btn').forEach(function (button) {
            button.addEventListener('click', function () {
                ...
                toggleStatus(userId, currentStatus, this);
            });
        });
    ...
    function toggleStatus(userId, currentStatus, buttonElement) {
        ...
        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4 && xhr.status === 200) {
                ...
                if (responseJson.success) {
                    ...
                    var newClass = newStatus === '<%= Status.ACTIVE.name() %>' ? "btn-danger" : "btn-success";
                    var newText = newStatus === '<%= Status.ACTIVE.name() %>' ? "Block" : "Unblock";
                    var newColor = newStatus === '<%= Status.ACTIVE.name() %>' ? "red" : "limegreen";
                    buttonElement.className = "btn " + newClass + " status-btn";
